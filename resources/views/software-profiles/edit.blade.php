<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Edit Software Profile: ') . $softwareProfile->name }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <form method="POST" action="{{ route('software-profiles.update', $softwareProfile->id) }}" class="p-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" id="name" name="name" value="{{ old('name', $softwareProfile->name) }}" required autofocus class="block mt-1 w-full border-gray-300 rounded-md shadow-sm">
                        </div>

                        <!-- Description -->
                        <div>
                            <label for="description" class="block font-medium text-sm text-gray-700">Description (Optional)</label>
                            <textarea id="description" name="description" rows="3" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm">{{ old('description', $softwareProfile->description) }}</textarea>
                        </div>
                                ...
                                @forelse ($softwareItems as $item)
                                    <label class="flex items-center">
                                        <input type="checkbox" name="software[]" value="{{ $item->id }}" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500"
                                            @if(in_array($item->id, old('software', $assignedSoftwareIds))) checked @endif
                                        >
